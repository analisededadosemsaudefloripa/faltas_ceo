...
Gráfico de radar apresentando o percentual das faltas por especialidade nos anos de 2014 até 2017, para as nove especialidades, sem distinção por variáveis (escolaridade, sexo, dia da semana, periodo de atendimento, unidade referência, idade), ou seja o valor total.

Para calcular, é adicionada uma coluna preenchida com valores 1, que serve para ajudar a agregar as faltas e não faltas, e somar a contagem por especialidade. Depois foi feito o calculo da percentual de faltantes por especialidade, (FALTAS / (FALTAS + NÃO FALTAS)) * 100. E por fim, o resultado gerando o gráfico de radar.
```{r}
...
#Separando o banco
Foram removidos da coluna DATA_ATD (que estava no formato: dd/mm/aaaa), o dia e o mês, resultando em apenas (aaaa), com o propósito de cortar o banco apenas para os anos de 2016 e 2017;
E depois separados em nove novos bancos, sendo um banco para cada especialidade.
É aplicada a função na.omit() nos três bancos que serão usados na análise, para remover os valores NA (not available).
```{r}
...
Gráfico de radar apresentando o percentual das faltas por especialidade nos anos de 2016 e 2017, para as nove especialidades, também sem distinção por variáveis (escolaridade, sexo, dia da semana, periodo de atendimento, unidade referência, idade), ou seja o valor total.

Foi feita uma análise para identificar o maior precentual de faltas por especialidade nos anos de 2016 e 2017.
As únicas variáveis necessárias foram ESPECIALIDADE e FALTAS, porém foi preciso criar uma coluna VALOR (preenchida apenas com o número 1), para que fosse possível agregar os valores de falta por especialidade e somar a quantidade final, o resultado foi transformado em duas novas colunas com os valores de FALTA.S e FALTA.N, o percentual foi calculado, (FALTA.S / (FALTA.S + FALTA.N) * 100) e atribuido a uma nova coluna, gerando também um gráfico ordenado pelo maior valor. A partir disso, foi decidido usar os bancos das três primeiras especialidades com maior percentual de faltas, que são: Odonto_EstomatologiaBucal, Odonto_Radiologica, Odonto_Perio, para aplicar os algoritmos de machine learning, porém o banco da EstomatologiaBucal tem uma quantidade de dados pequena (850 linhas), e por conta disso não possui muita variância entre os dados de algumas variáveis, o que acabou gerando erros nos testes com os algoritmos, e portanto foi substituido pelo quarto banco na sequência, Odonto_Endodontia.
```{r}
...
Para cada um dos bancos escolhidos, que são subconjuntos do banco de dados das faltas nos anos de 2016 e 2017 (faltas_ceo_16_17), foi feita a análise de algumas variáveis (DIA_SEMANA, SEXO, ESCOLARIDADE, PERIODO_ATD) de modo independente, para isso foram criados novos subconjuntos para cada um dos bancos escolhidos em relação a variável a ser analisada, de modo que eram gerados bancos de apenas duas colunas ('Variável' e 'Valor correspondente'), sem relação nehuma com qualquer outra variável.

#DIA_SEMANA
Para cada um dos três bancos escolhidos (Odonto_Endodontia, Odonto_Radiologica, Odonto_Perio), foi calculado o percentual de faltas por dia da semana. No eixo X: o valor percentual de faltas, no eixo Y: o dia da semana. O gráfico mostra em ordem decrescente o percentual de faltas.
```{r}
#########################################################################################################################
...
#SEXO
Para cada um dos três bancos escolhidos (Odonto_Endodontia, Odonto_Radiologica, Odonto_Perio), foi calculado o percentual de faltas por sexo. No eixo X: o valor percentual de faltas, no eixo Y: o sexo dos faltantes. O gráfico mostra em ordem decrescente o percentual de faltas.
```{r}
#########################################################################################################################
#Odonto_Endodontia por dia da semana
...
#ESCOLARIDADE
Para cada um dos três bancos escolhidos (Odonto_Endodontia, Odonto_Radiologica, Odonto_Perio), foi calculado o percentual de faltas por escolaridade. No eixo X: a escolaridade dos faltantes, no eixo Y: o percentual dos faltantes. O gráfico mostra em ordem decrescente o percentual de faltas.
```{r}
#########################################################################################################################
#Odonto_Endodontia por dia da semana
...
#PERIODO_ATD
Para cada um dos três bancos escolhidos (Odonto_Endodontia, Odonto_Radiologica, Odonto_Perio), foi calculado o percentual de faltas por periodo de atendimento. No eixo X: o valor percentual de faltas, no eixo Y: o periodo de atendimento. O gráfico mostra em ordem decrescente o percentual de faltas.
```{r}
#########################################################################################################################
#Odonto_Endodontia por dia da semana
...
ml_models <- function(nome_banco){
  ...
  banco_escolhido <- na.exclude(nome_banco)
  banco_escolhido <- na.omit(nome_banco)

  banco_escolhido$FALTA <- as.factor(banco_escolhido$FALTA)
  ...
  while(i <= length((modelos))) {
    ...
    #espec <- b[2] %>% as.data.frame()
    #espec <- espec$Specificity
